...
public class GizmoTranslateScript : MonoBehaviour {
    ...
    public void Awake() {
        ...
        transform.position = new Vector3(0, 0, 0);//translateTarget.transform.position;
    }
    ...
    public void Update() {
        for (int i = 0; i < 3; i++) {
            if (Input.GetMouseButton(0) && detectors[i].pressing) {
                FindObjectOfType<CameraMovement>().gizmoSelected = true;
                    ...
                    case 0:
                        {
                            ...
                            if (detectors[i].pressingPlane) {
                                float deltaY = Input.GetAxis("Mouse Y") * (Time.deltaTime * distance * speed);
                                offset = Vector3.up * deltaY;
                                offset = new Vector3(0.0f, offset.y, 0.0f);
                                translateTarget.transform.Translate(offset);

                                float deltaZ = Input.GetAxis("Mouse X") * (Time.deltaTime * distance * speed);
                                offset = Vector3.forward * deltaZ;
                                offset = new Vector3(0.0f, 0.0f, offset.z);
                                translateTarget.transform.Translate(offset);

                            } else {
                                float delta = Input.GetAxis("Mouse X") * (Time.deltaTime * distance * speed);
                    ...
                    case 1:
                        {
                            ...
                            if (detectors[i].pressingPlane) {
                                float deltaX = Input.GetAxis("Mouse X") * (Time.deltaTime * distance * speed);
                                offset = Vector3.left * deltaX;
                                offset = new Vector3(offset.x, 0.0f, 0.0f);
                                translateTarget.transform.Translate(offset);

                                float deltaZ = Input.GetAxis("Mouse Y") * (Time.deltaTime * distance * speed);
                                offset = Vector3.forward * deltaZ;
                                offset = new Vector3(0.0f, 0.0f, -offset.z);
                                translateTarget.transform.Translate(offset);

                            } else {
                                float delta = Input.GetAxis("Mouse Y") * (Time.deltaTime * distance * speed);
            ...
            {
                FindObjectOfType<CameraMovement>().gizmoSelected = false;
            }
        }
    }
